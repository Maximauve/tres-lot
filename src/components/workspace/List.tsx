				...
				<tbody>
					{items.map((item) => (
						<tr key={item["@id"]}>
							...
							<td>
								<Links
									items={item["tables"] ? item["tables"].map((ref: any) => ({
										href: `/les/show/${encodeURIComponent(ref)}`,
										name: ref,
									})) : ""}
								/>
							</td>
							...
							<td>
								<Link
									to={`/workspaces/show/${encodeURIComponent(item["@id"])}`}
								>
							...
							<td>
								<Link
									to={`/workspaces/edit/${encodeURIComponent(item["@id"])}`}
								>
...
const List = () => {
	...
	const id = (page && decodeURIComponent(page)) || "kspaces";
